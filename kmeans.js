...
var centroideo = [];
var ndim = 4;
// Functon that creates the elements of the code (centroides and points )
function crea()
{
    var punti = [];
    for (var i = 0; i < dataset.length; i++) //points creation
    {
        punti.push({
    ...
    for (var i = 0; i < dataset.length; i++) //calculating the distance between points and centroids
    {
        if(ndim == 4)
        {
    ...
    var centroide = [];
    for (var i = 0; i < 3; i++) //centroids creation
    {
        centroide.push({
...
function disegna(punti, centroide)  //drawing the centroids and the points
{
    ...
    ctx.fill();
    //Assigning the cluster's color to the points associated with it
    for (var i = 0; i < punti.length; i++) 
    {
        ctx.beginPath();
        ctx.arc(punti[i].x, punti[i].y, 4, 0, 2 * Math.PI);
...
{
    //Assigning points to the centroids
    for (var i = 0; i < punti.length; i++) 
    {
    ...
    //Update of the centroids' position
    ...
    //Checking if the centroids don't move 
